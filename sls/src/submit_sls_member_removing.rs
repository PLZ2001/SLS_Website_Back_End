...
pub struct SlsMemberRemoving {
    pub student_ids: Vec<String>,
}

#[derive(Debug)]
pub struct FailedToSubmitSlsMemberRemoving(Box<String>);
                                            ...
                                            match token::validate_token(&user.token).await {
                                                Ok(validation_result) => {
                                                    if validation_result {
                                                        ...
                                                        let filter = doc! {"student_id": {"$in":sls_member_removing.student_ids.clone()}};
                                                        ...
                                                        match collection.find(filter.clone(), find_options).await {
                                                            Ok(cursor) => {
                                                                    ...
                                                                    match cursor_enumerate.next().await {
                                                                        Some(find_result) => {
                                                                            match find_result.1 {
                                                                                ...
                                                                                Err(e) => {
                                                                                    return Err(warp::reject::custom(FailedToSubmitSlsMemberRemoving(Box::new(e.kind.to_string()))));
                                                                                }
                                                                            }
                                                                        }
                                                                        None => {
                                                                ...
                                                                if counter == sls_member_removing.student_ids.len() {
                                                                    let filter = doc! {"student_id": {"$in":sls_member_removing.student_ids.clone()}};
                                                ...
                                                Err(e) => {
                                                    return Err(warp::reject::custom(FailedToSubmitSlsMemberRemoving(Box::new(e))));
                                                }
                                            }
                                        }
                                        None => {
