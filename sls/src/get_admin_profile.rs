...
#[derive(Debug)]
pub struct FailedToGetAdminProfile(Box<String>);

impl warp::reject::Reject for FailedToGetAdminProfile {}

pub async fn fun_get_admin_profile(token: Option<String>) -> Result<warp::reply::Json, warp::Rejection> {
    match token {
        Some(token) => {
                    ...
                    match Client::with_options(client_options) {
                        Ok(client) => {
                            ...
                            match collection.find_one(filter, None).await {
                                    ...
                                    match find_result {
                                        Some(user) => {
                                            match token::validate_token(&user.token).await {
                                                ...
                                                Err(e) => {
                                                    return Err(warp::reject::custom(FailedToGetAdminProfile(Box::new(e))));
                                                }
                                            }
                                        }
                                        None => {
                                ...
                                Err(e) => {
                                    return Err(warp::reject::custom(FailedToGetAdminProfile(Box::new(e.kind.to_string()))));
                                }
                            }
                        }
                        Err(e) => {
